...
var ${operate["action"]?replace('/','_')} = {action:"${operate["action"]+actionSuffix}"<#if operate['confirm']??>,msg:"${operate['confirm']}"</#if><#if operate['type']??>,type:"${operate['type']}"</#if><#if operate['define']??>,define:${operate['define']}</#if>};
  </#list>
...
<#if pageList?? && pageList?size gt 0>
	...
	<tr onmouseover="Pn.LTable.lineOver(this);" onmouseout="Pn.LTable.lineOut(this);" onclick="Pn.LTable.lineSelect(this);">
		...
		<#list normalOperate as operate>
			...
			<#if show>
				<@p.operateRight operate=operate['action'] checkRight=checkRight><a<#if opDisabled> disabled="disabled" style="background-color:#CCCCCC;"<#else> href="javascript:_operate(${operate['action']?replace('/','_')},'${row[keyId]}','${row[keyId2]}');"</#if> class="pn-loperator">
				<#if operate['ifcheck']??>
