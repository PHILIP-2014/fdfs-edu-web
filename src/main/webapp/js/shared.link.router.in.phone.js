 ...
 * date:2014/11/11
 */
define('shared.link.router.in.phone',['parox'],function(require,exports){
    'use strict';
    ...
    var folderThumbnailTpl='<%var link=model.getSharedLink()%>' +
                            ...
                            '</h4>';
    var folderListTpl='<%var link=model.getSharedLink();%>' +
                        '<div class="media" data-type="folder">' +
                            '<span class="pull-left icon-40 icon-40-folder-<%=model.getICON()%>"></span>' +
                            '<div class="media-body">' +
                                '<h4>' +
                                    '<a href="<%=link%>" class="text-main" data-name="name" data-behavior="folder-click"><%=model.get(\'folderName\')%></a>' +
                                    '<div class="clearfix">' +
                                        '<div class="pull-left"><%=model.formatLastModifyTime()%></div>' +
                                    '</div>' +
                                '</h4>' +
                            '</div>' +
                        '</div>';
    var fileThumbnailTpl='<div class="p-t-25 text-center">' +
                        ...
                        '</h4>';
    var fileListTpl='<div class="media">' +
                        '<span class="pull-left icon-40 icon-40-unknow icon-40-<%=model.get(\'suffix\')%>"></span>' +
                        '<div class="media-body">' +
                            '<h4>' +
                                '<a href="javascript:;" class="text-main" data-name="name"><%=model.get(\'docName\')%></a>' +
                                '<div class="clearfix">' +
                                    '<div class="pull-left"><%=model.formatLastModifyTime()%></div>' +
                                    '<div class="pull-right"><%=model.formatFileSize()%></div>' +
                                '</div>' +
                            '</h4>' +
                        '</div>' +
                    '</div>';
    ...
    var FolderItemView=PAROX.View.extend({
        ...
        initialize:function(option){
            var className=this.parent.get('viewType')==='thumbnails'?
                ...
                'p-15';
            this.$el.addClass(className).attr('data-behavior','menu-item').attr('data-command','download');
        },
        render:function(){
        ...
        events:{
            ...
            'click [data-behavior="folder-click"]':'onFolderClick'
        },
        onMenuItemClick:function(e){
            var $this=this.$el;
            if($this.find('[data-type="folder"]').length>0){
                var hash=$this.find('[data-behavior="folder-click"]').attr('href');
                window.location.hash=hash;
                return;
            }
    ...
    var FileView=PAROX.View.extend({
        ...
        renderFolderView:function(){
            ...
            var total=response.folderCondition.total;
            var text='<span class="f-s-12"> (共'+total+'个文件)</span>';
            this.$('[data-behavior="folder-name"]').html(folder.get('folderName')+text);
